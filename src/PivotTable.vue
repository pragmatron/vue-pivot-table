      ...
      <thead>
        ...
        >
          ...
          >
            ...
            <template v-else>{{ col[colFieldIndex] }}</template>
          </th>
          <!-- Top right dead zone -->
          <th
      ...
      <tbody>
        <tr v-for="(row, rowIndex) in rows" :key="JSON.stringify(row)">
          ...
          <th
            ...
            v-if="(rowField.showHeader === undefined || rowField.showHeader) && spanSize(rows, rowFieldIndex, rowIndex) !== 0"
          >
            <slot
            ...
            <template v-else>{{ row[rowFieldIndex] }}</template>
          </th>
          <!-- Values -->
          <td v-for="col in cols" :key="JSON.stringify(col)" class="text-right">
            <slot
              ...
              name="value"
              v-bind:value="values[JSON.stringify({ col, row })]"
            />
          ...
          <th
            ...
            :key="rowField.key"
            :rowspan="spanSize(rows, rowFields.length - rowFieldIndex - 1, rowIndex)"
            v-if="rowField.showFooter && spanSize(rows, rowFields.length - 1 - rowFieldIndex, rowIndex) !== 0"
          >
            <slot
              v-if="rowField.footerSlotName"
              :name="rowField.footerSlotName"
              v-bind:value="row[rowFields.length - rowFieldIndex - 1]"
            >
              Missing slot
              <code>{{ rowField.footerSlotName }}</code>
            </slot>
            <template v-else>{{ row[rowFields.length - rowFieldIndex - 1] }}</template>
          </th>
        </tr>
      </tbody>
      ...
      <tfoot>
        ...
        >
          ...
          <th
            v-for="(col, colIndex) in cols"
            :key="JSON.stringify(col)"
            :colspan="spanSize(cols, colFields.length - colFieldIndex - 1, colIndex)"
            v-if="spanSize(cols, colFields.length - colFieldIndex - 1, colIndex) !== 0"
          >
            <slot
              v-if="colField.footerSlotName"
              :name="colField.footerSlotName"
              v-bind:value="col[colFields.length - colFieldIndex - 1]"
            >
              Missing slot
              <code>{{ colField.footerSlotName }}</code>
            </slot>
            <template v-else>{{ col[colFields.length - colFieldIndex - 1] }}</template>
          </th>
  ...
  computed: {
    cols: function() {
      ...
      const extractColsRecursive = (depth, filters) => {
        ...
        const values = [
          ...new Set(
            this.filteredData({ data: this.data, colFilters: filters }).map(
              item => getter(item)
            )
          )
        ].sort(sort);
    ...
    rows: function() {
      ...
      const extractRowsRecursive = (depth, filters) => {
        ...
        const values = [
          ...new Set(
            this.filteredData({ data: this.data, rowFilters: filters }).map(
              item => getter(item)
            )
          )
        ].sort(sort);
  ...
  methods: {
    ...
    spanSize: function(values, fieldIndex, valueIndex) {
      ...
      if (
        ...
        (fieldIndex === 0 ||
          this.spanSize(values, fieldIndex - 1, valueIndex) === 0)
      ) {
        return 0;
      }
      ...
      while (
        ...
        (fieldIndex === 0 ||
          (i + 1 < values.length &&
            this.spanSize(values, fieldIndex - 1, i + 1) === 0))
      ) {
        i++;
        size++;
      }
